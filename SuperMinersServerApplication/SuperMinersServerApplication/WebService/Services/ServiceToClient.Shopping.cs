...
namespace SuperMinersServerApplication.WebService.Services
{
    ...
    {
        public PlayerBuyVirtualShoppingItemRecord[] GetPlayerBuyVirtualShoppingItemRecord(string token, int userID, int itemID, MyDateTime beginBuyTime, MyDateTime endBuyTime, int pageItemCount, int pageIndex)
        {
            if (RSAProvider.LoadRSA(token))
            {
                try
                {
                    return VirtualShoppingController.Instance.GetPlayerBuyVirtualShoppingItemRecordByID(userID, itemID, beginBuyTime, endBuyTime, pageItemCount, pageIndex);
                }
                catch (Exception exc)
                {
