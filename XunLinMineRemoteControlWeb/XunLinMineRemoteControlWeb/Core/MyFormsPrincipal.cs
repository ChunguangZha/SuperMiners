       ...
       where TUserData : class, new()
    {
        ...
        public static void SignIn(string loginName, TUserData userData, int expiration)
        {
            try
            {
                ...
                string data = null;
                if (userData != null)
                    data = DESEncrypt.EncryptDES((new JavaScriptSerializer()).Serialize(userData));


                // 2. 创建一个FormsAuthenticationTicket，它包含登录名以及额外的用户数据。
                FormsAuthenticationTicket ticket = new FormsAuthenticationTicket(
                    2, loginName, DateTime.Now, DateTime.Now.AddHours(1), true, data);
            ...
            {
                LogHelper.Instance.AddErrorLog("SignIn Exception loginName: " + loginName, exc);
            }
        }
        ...
        public static void TrySetUserInfo(HttpContext context)
        {
            ...
            try
            {
                ...
                if (ticket != null && string.IsNullOrEmpty(ticket.UserData) == false)
                    // 3. 还原用户数据
                    userData = (new JavaScriptSerializer()).Deserialize<TUserData>(DESEncrypt.DecryptDES(ticket.UserData));
